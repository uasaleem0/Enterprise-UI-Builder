...
function Import-IABlock {
    ...
    if ($InputFile) {
        try {
            $content = Get-Content -Path $InputFile -Raw -Encoding UTF8 -ErrorAction Stop
            Write-Host ""; Write-Host "Reading IA from file: $InputFile" -ForegroundColor Gray; Write-Host ""
        } catch {
            Write-Host "[ERROR] Failed to read IA file: $InputFile" -ForegroundColor Red
            return
        }
    }
