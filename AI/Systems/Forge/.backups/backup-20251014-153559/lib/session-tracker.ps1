...
function Track-FileModification {
    param(
        [string]$ProjectPath,
        [string]$FileName
    )
    ...
    if (-not $state.current_session) { return }

    if ($FileName -notin $state.current_session.files_modified) {
        $state.current_session.files_modified += $FileName
        Set-ProjectState -ProjectPath $ProjectPath -State $state
    }
}

function Track-DeliverableChange {
    param(
        [string]$ProjectPath,
        [string]$Deliverable,
        [int]$OldValue,
    ...
    $exists = $state.current_session.deliverables_changed | Where-Object { $_.deliverable -eq $Deliverable }
    if (-not $exists) {
        $state.current_session.deliverables_changed += $change
    }
