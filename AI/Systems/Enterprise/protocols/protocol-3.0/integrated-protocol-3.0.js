...
class IntegratedProtocol30 {
      ...
      try {
        ...
        if (phaseNumber <= 10) {
          ...
          if (phaseNumber === 10 && this.validationAnalyzer) {
            const v = await this.validationAnalyzer.executePhase10(options.localUrl || process.env.ENT_LOCAL_URL || 'http://localhost:3000');
            if (!v.passed) {
    ...
    while (iterations < maxIterations && similarity < phase.targetSimilarity) {
      ...
      try {
        ...
        if (phaseNumber >= 2) { // Skip validation for design brief phase
          ...
          const ev = await this.captureEvidenceIfPossible(options.localUrl || process.env.ENT_LOCAL_URL || 'http://localhost:3000', targetUrl, evidenceDir);
          const validation = await this.visualValidator.runComprehensiveValidation(
            options.localUrl || process.env.ENT_LOCAL_URL || 'http://localhost:3000',
            targetUrl,
          ...
          // CRITICAL_FAIL if no improvement across 10 consecutive iterations
          if (typeof this.__noImprove === 'undefined') this.__noImprove = 0;
          if (typeof delta !== 'undefined' && delta < 1) this.__noImprove++; else this.__noImprove = 0;
          if (this.__noImprove >= 10) {
            issues.push('CRITICAL_FAIL: No improvement across 10 consecutive iterations');
            console.log('   ï¿½?O CRITICAL_FAIL: No improvement across 10 consecutive iterations');
            break;
          }
  ...
  // Evidence helpers
  async ensureEvidenceDir(projectPath, phaseNumber, iteration) {
    const fs = require('fs').promises;
    const path = require('path');
    const dir = path.join(projectPath, 'evidence', `phase-${phaseNumber}`, `iter-${iteration}`);
    await fs.mkdir(dir, { recursive: true });
    return dir;
  }

  async captureEvidenceIfPossible(localUrl, targetUrl, outDir) {
    const path = require('path');
    let playwright;
    try { playwright = require('playwright'); } catch { return { baselinePath: null, candidatePath: null }; }
    try {
      const browser = await playwright.chromium.launch();
      const context = await browser.newContext({ viewport: { width: 1440, height: 900 } });
      const page = await context.newPage();
      const candidatePath = path.join(outDir, 'local-candidate.png');
      await page.goto(localUrl, { waitUntil: 'load' });
      await page.screenshot({ path: candidatePath, fullPage: true });

      const page2 = await context.newPage();
      const baselinePath = path.join(outDir, 'target-baseline.png');
      await page2.goto(targetUrl, { waitUntil: 'load' });
      await page2.screenshot({ path: baselinePath, fullPage: true });

      await browser.close();
      return { baselinePath, candidatePath };
    } catch (e) {
      return { baselinePath: null, candidatePath: null };
    }
  }

  async generateDesignBriefArtifact(projectPath, targetUrl) {
    try {
      const { writeDesignBrief } = require('../../systems/Enterprise/extract-design-brief.js');
      const res = await writeDesignBrief(targetUrl, projectPath);
      if (res && res.mdPath) {
        console.log(`Design Brief saved: ${res.mdPath}`);
      } else {
        console.log('Design Brief: failed to generate (using extractor)');
      }
    } catch (e) {
      console.log('Design Brief generation error:', e.message || String(e));
    }
  }
