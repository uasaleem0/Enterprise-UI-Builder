...
function checkDocumentationStatus() {
  ...
  requiredDocs.forEach(doc => {
    try {
      let exists = fs.existsSync(path.join('.', doc));
      // Fallbacks for command reference relocations
      if (!exists && doc === 'commands/command-reference.md') {
        exists = fs.existsSync(path.join('.', 'docs/command-reference.md')) ||
                 fs.existsSync(path.join('.', 'command-reference.md'));
      }
