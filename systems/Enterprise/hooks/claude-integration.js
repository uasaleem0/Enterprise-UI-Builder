...
console.log = function(...args) {
  ...
  // Monitor all console output for patterns
  if (global.metaAnalyst) {
    global.metaAnalyst.detect(text, { tool: 'console' });
  }
...
process.cwd = function() {
  ...
  // Auto-activate if entering Enterprise context
...
fs.readFileSync = function(path, ...args) {
  if (global.metaAnalyst && path.includes('Enterprise')) {
    global.metaAnalyst.detect(`Reading file: ${path}`, { tool: 'file_read' });
  }
...
fs.writeFileSync = function(path, data, ...args) {
  if (global.metaAnalyst && path.includes('Enterprise')) {
    global.metaAnalyst.detect(`Writing file: ${path}`, { tool: 'file_write' });

    // Monitor file content for patterns
    if (typeof data === 'string') {
      global.metaAnalyst.detect(data, { tool: 'file_content' });
    }
  }
